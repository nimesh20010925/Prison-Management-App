...
function LethelUpdate() {
    ...
    useEffect(() => {
        const fetchHandler = async () => {
            try {
                const response = await axios.get(`http://localhost:3500/lethel/${id}`);
                setInputs(response.data.lethel); // Assuming response.data.firearm contains the firearm data
            } catch (error) {
                console.error(error);
            }
        };
    ...
    const sendRequest=async()=>{
        ...
        await axios.put(`http://Localhost:3500/lethel/${id}`,{
